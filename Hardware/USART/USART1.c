...
USART1_Status_t USART1_SendByteTimeout(uint8_t data, uint32_t timeout)
{
    ...
    TIM2_Start(&timer, 2 * timeout); // 启动定时器(测试发现定时1s比实际时间快1s，所以这里就大概填为2s了*By CYZ)
    // 等待发送缓冲区为空
    ...
    // 等待发送完成
    TIM2_Start(&timer, 2 * timeout); // 重新启动定时器
...
USART1_Status_t USART1_ReceiveArray(uint8_t *array, uint16_t *length, uint32_t timeout)
{
    ...
    uint16_t received = 0;
    TIM2_Start(&timer, 2 * timeout); // 启动定时器(测试发现比如定时1s比实际时间快1s，所以这里就大概填为2s了*By CYZ)
