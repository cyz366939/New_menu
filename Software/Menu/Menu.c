...
void Menu_DisplayStatusBar(void)
{
    ...
    // 构建菜单路径（简化版：只显示当前菜单名称）
    if(menu != NULL && menu->name != NULL)
    {
        uint8_t len = strlen(menu->name);
        if(len > 18) len = 18;  // 留出空间给LOGO
    ...
    // 显示状态栏（第一行，使用6x8字体以显示更多字符）
    OLED_ShowString(0, 0, status_str, OLED_8X16);
    OLED_ShowChinese(110, 0, "蔕");
}
